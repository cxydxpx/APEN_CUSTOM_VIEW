...
import com.example.apen.apencustomview.ToolMath;
...
public class CurveView extends View {
     ...
     * 左边文字
     */
    private float[] mLeftText = {2.46f, 2.45f, 2.44f, 2.43f, 2.42f, 2.41f, 2.40f};
    /**
     * 底部文字
     */
    private String[] day = {"礼拜一", "礼拜一", "礼拜一", "礼拜一", "礼拜一", "礼拜一", "礼拜一"};
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mWidth = (float) (getWidth() - getWidth() * 0.1);//45
        mHeight = (float) (getHeight() - getHeight() * 0.1);//90
        mRectHeight = (float) (getHeight() - getHeight() * 0.1);//90
        mPaintRectWidth = (float) (mWidth * 0.8 / mCount);//8
    ...
    private void onDrawLine(Canvas canvas) {
        ...
        for (float i = 0; i < 7; i++) {
            ...
            if (i == 0) {
                ...
                float size = mTextPaint.measureText(mLeftText[(int) i] + "");
                //绘制线
                canvas.drawLine(mPaintRectWidth + mPaintRectWidth / 2, height, getWidth(), height, mTextPaint);
                //绘制左边Y轴的文字
                canvas.drawText(mLeftText[(int) i] + "", (float) (mPaintRectWidth - mPaintRectWidth * 0.35), height + size / 5, mTextPaint);
            }
